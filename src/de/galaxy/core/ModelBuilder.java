...
public class ModelBuilder 
{
	...
	public static Model CreatePlane(GL4 gl, Texture2D[] textures, boolean generateTangentSpace) 
	{
		...
		{
			float[] tangentSpace = new float[((planeVertices.length/8)/3)*3];
			int verticesCount = 0;
			...
			for(int i = 0; i < planeVertices.length; i++) 
			{
				...
				if(vertexCount == 0) 
				{
					positions[verticesCount].X = planeVertices[i];
				}
				else if(vertexCount == 1) 
				{
					positions[verticesCount].Y = planeVertices[i];
				}
				else if(vertexCount == 2) 
				{
					positions[verticesCount].Z = planeVertices[i];
				}
				else if(vertexCount == 3) 
				{
					normals[verticesCount].X = planeVertices[i];
				}
				else if(vertexCount == 4) 
				{
					normals[verticesCount].Y = planeVertices[i];
				}
				else if(vertexCount == 5) 
				{
					normals[verticesCount].Z = planeVertices[i];
				}
				else if(vertexCount == 6) 
				{
					textCoords[verticesCount].X = planeVertices[i];
				}
				else if(vertexCount == 7) 
				{
					textCoords[verticesCount].Y = planeVertices[i];
			...
			float[] temp = new float[planeVertices.length+(tangentSpace.length*3)];
			int triangles = (planeVertices.length/8)/3;
			...
			for(int i = 0; i < triangles; i++) 
			{
				for(int k = i*3; k < (i+1)*3; k++) 
				{
					...
					{
						temp[tempCounter] = planeVertices[j];
						tempCounter++;
					}
