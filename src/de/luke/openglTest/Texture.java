...
public class Texture 
{
	...
	public float highlightFocus = 32;
	
	public Texture(String path, boolean flipImage) 
	{
		this.path = path;
		load(flipImage);
	}
	
	public Texture (String path, GL4 gl, ShaderProgram shader, int textureCount, int textureType, float highlightFocus) 
	{
		this.path = path;
	...
	private void loadIntoGraphicsCard(GL4 gl, ShaderProgram shader, int textureCount) 
	{
		if(loadSuccess) 
		{
			...
			case Texture.TEXTURE_DIFFUSE:
				textNameArr = Texture.DIFFUSE_STRINGS;
				arrayCount = diffuseCount;
				diffuseCount++;
				break;
			case Texture.TEXTURE_EMISSION:
				textNameArr = Texture.EMISSION_STRINGS;
				arrayCount = emissionCount;
				emissionCount++;
				break;
			case Texture.TEXTURE_SPECULAR:
				textNameArr = Texture.SPECULAR_STRINGS;
